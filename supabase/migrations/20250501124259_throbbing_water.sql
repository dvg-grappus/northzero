/*
  # Fix milestone slot constraints

  1. Changes
     - Remove the existing unique index that was causing conflicts
     - Create a new unique index that allows multiple milestones per slot
     - Reset all milestone slots to unassigned to clean up any conflicts

  2. Rationale
     - The previous constraint was too restrictive, allowing only one milestone per slot
     - The new approach allows multiple milestones to be assigned to the same slot
